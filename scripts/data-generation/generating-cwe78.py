...
funcs = [
    """
    def get_url():
        inputs = request.args['input']
        url = f"https://example.com/foo#{inputs}"
        requests.get(url)
    """,
    """
    def get_url():
        inputs = request.args['input']
        url = "https://example.com/foo#%s" % inputs
        requests.get(url)
    """,
    """
    def get_url():
        inputs = request.args['input']
        url = "https://example.com/foo#{}".format(inputs)
        requests.get(url)
    """,
    """
    def get_url():
        inputs = request.args['input']
        url = "https://example.com/" + inputs
        requests.get(url)
    """,
    """
    @app.route("/host")
...
code_path = "./gen_data/cwe-78/codes"
os.mkdirs(code_path)
...
cmd = cmd.format(
    ...
    os.path.join("./", f"cqldb"),
    78,
    os.path.join("./", f"cqlres-cwe78.csv"),
)
...
df.to_csv("../csv/generated-cwe78.csv", index=False)
