...
df = pd.read_csv("./csv/generated-cwe20.csv")
...
df["has_vul_func20"] = (
    df["vul_func20_codeql"].astype(str).parallel_apply(lambda x: "Func" in x)
)
mal_df = df.loc[df["has_vul_func20"] == True].copy().reset_index(drop=True)
...
for i in range(mal_df.shape[0]):
    ...
    vul_loc = mal_df.at[i, "vul_func20_codeql"]
...
print(df_m.shape)
df_m.to_csv("csv/pws/df_codeql_m20_processed.csv", index=False)
ben_df = df.loc[(df["codeql-cwe20"] == False)].copy().reset_index(drop=True)
...
ben_df["func_ben20"] = ben_df["generated_code"].parallel_apply(
    lambda x: detect_function_from_path(x, tokenizer=tokenizer)
)
ben_df = ben_df.loc[ben_df["func_ben20"] != "N/A"].copy().reset_index(drop=True)
ben_df.shape
...
for i in range(ben_df.shape[0]):
    ...
    vul_loc = ben_df.at[i, "func_ben20"]
    for loc in vul_loc.split("|"):
        f_name = loc.split("-")[0]
...
df_b.to_csv("csv/pws/df_codeql_b20_processed.csv", index=False)
