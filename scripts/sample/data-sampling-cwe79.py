...
df = pd.read_csv("./csv/generated-cwe-79.csv")
...
name = []
vul_func = []
path = "./gen_data/cwe-79/codes/"
df_res = pd.read_csv("./gen_data/cwe-79/codeql-cwe79.csv", header=None)
...
df["codeql-cwe79"] = 0
df["vul_func79_codeql"] = "N/A"
df["id"] = range(df.shape[0])

res_df = pd.DataFrame({"new_name": name, "vul_func79_codeql": vul_func})
res_df["codeql-cwe79"] = 1
...
update_df = update_df.drop(["codeql-cwe79", "vul_func79_codeql"], axis=1)
...
df["has_vul_func79"] = (
    df["vul_func79_codeql"].astype(str).parallel_apply(lambda x: "Func" in x)
)
mal_df = df.loc[df["has_vul_func79"] == True].copy().reset_index(drop=True)
...
for i in range(mal_df.shape[0]):
    ...
    vul_loc = mal_df.at[i, "vul_func79_codeql"]
...
df_m.to_csv("csv/pws/df_codeql_m79_processed.csv", index=False)
ben_df = df.loc[(df["codeql-cwe79"] == False)].copy().reset_index(drop=True)
...
ben_df["func_ben79"] = ben_df["generated_code"].parallel_apply(
    lambda x: detect_function_from_path(x, tokenizer=tokenizer)
)
ben_df = ben_df.loc[ben_df["func_ben79"] != "N/A"].copy().reset_index(drop=True)
...
for i in range(ben_df.shape[0]):
    ...
    vul_loc = ben_df.at[i, "func_ben79"]
    for loc in vul_loc.split("|"):
        f_name = loc.split("-")[0]
...
df_b.to_csv("csv/pws/df_codeql_b79_processed.csv", index=False)
